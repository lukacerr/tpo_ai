...
import { Amenity } from '@tpoai/data-commons';
import { PatchBuildingRequest } from '@/requests/buildings.requests';

interface IAmenityList {
  list?: { address?: string; amenities?: Amenity[] }[];
  actions?: boolean;
  forceBuilding?: boolean;
}

export default function AmenitiesList({ list, actions, forceBuilding }: React.PropsWithRef<IAmenityList>) {
  const values = list
    ?.filter((b) => b.amenities && b.amenities.length)
    .flatMap((b) => b.amenities?.map((a) => ({ ...a, buildingAddress: b.address })));

  const reloadPage = () => location.reload();

  const deleteV = (row: Amenity) => {
    ...
    <Box sx={{ width: '100%' }}>
      ...
      >
        ...
        >
          <thead>
            <tr>
              <th style={{ width: 'var(--Table-firstColumnWidth)' }}>Piso</th>
          ...
          <tbody>
            {values.map((row) => (
              <tr key={row?.id}>
                <td>{row?.floor || <i>No especificado</i>}</td>
                <td>{row?.name || '-'}</td>
